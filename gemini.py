...
CUSTOM_EMOJI_TEXT_RE = re.compile(r"\[\[ce:(\d+)\|(.+?)\]\]")
CUSTOM_EMOJI_BTN_RE = re.compile(r"^\s*\[\[btnce:(\d+)\]\]\s*(.*)$", re.DOTALL)


def render_custom_emoji_html(text: str, parse_mode: str = "Markdown") -> tuple[str, str]:
    """Ручной формат прем-эмодзи в тексте:
    [[ce:5368324170671202286|🙂]]
    """
    source = text or ""
    if not CUSTOM_EMOJI_TEXT_RE.search(source):
        return source, parse_mode

    html_text = html_escape(source)
    html_text = CUSTOM_EMOJI_TEXT_RE.sub(
        lambda m: f'<tg-emoji emoji-id="{m.group(1)}">{html_escape(m.group(2))}</tg-emoji>',
        html_text,
    )
    return html_text, "HTML"


def parse_button_custom_emoji(text: str) -> tuple[str, str | None]:
    """Ручной формат прем-эмодзи в кнопке:
    [[btnce:5368324170671202286]] Текст кнопки
    """
    raw = text or ""
    m = CUSTOM_EMOJI_BTN_RE.match(raw)
    if not m:
        return raw, None
    icon_id, label = m.group(1), (m.group(2) or "").strip()
    return (label or " "), icon_id


def _patch_keyboard_builders_for_custom_emoji():
    inline_orig = InlineKeyboardBuilder.button
    reply_orig = ReplyKeyboardBuilder.button

    def inline_patched(self, *args, **kwargs):
        ...
        if isinstance(text, str):
            label, icon_id = parse_button_custom_emoji(text)
            try:
                dyn_icon = get_button_emoji_id_for_label(label)
            except Exception:
                dyn_icon = None
            kwargs["text"] = label
            if icon_id and "icon_custom_emoji_id" not in kwargs:
                kwargs["icon_custom_emoji_id"] = icon_id
            elif dyn_icon and "icon_custom_emoji_id" not in kwargs:
                kwargs["icon_custom_emoji_id"] = dyn_icon
            try:
                register_button_label(label)
            except Exception:
                pass
        return inline_orig(self, *rest_args, **kwargs)

    def reply_patched(self, *args, **kwargs):
        text = kwargs.get("text")
        rest_args = args
        if text is None and args:
            text = args[0]
        ...
        if isinstance(text, str):
            ...
            try:
                ...
                pass
        return reply_orig(self, *rest_args, **kwargs)

    InlineKeyboardBuilder.button = inline_patched
    ReplyKeyboardBuilder.button = reply_patched


_patch_keyboard_builders_for_custom_emoji()


def _patch_bot_custom_emoji_methods():
    ...
    async def patched_send_message(*args, **kwargs):
        if "text" in kwargs:
            ...
            kwargs["text"], kwargs["parse_mode"] = render_custom_emoji_html(kwargs["text"], pm)
        return await orig_send_message(*args, **kwargs)

    async def patched_send_photo(*args, **kwargs):
        if "caption" in kwargs:
            pm = kwargs.get("parse_mode", "Markdown")
            kwargs["caption"], kwargs["parse_mode"] = render_custom_emoji_html(kwargs["caption"], pm)
        return await orig_send_photo(*args, **kwargs)

    async def patched_edit_message_text(*args, **kwargs):
        if "text" in kwargs:
            pm = kwargs.get("parse_mode", "Markdown")
            kwargs["text"], kwargs["parse_mode"] = render_custom_emoji_html(kwargs["text"], pm)
        return await orig_edit_message_text(*args, **kwargs)

    async def patched_edit_message_caption(*args, **kwargs):
        if "caption" in kwargs:
            pm = kwargs.get("parse_mode", "Markdown")
            kwargs["caption"], kwargs["parse_mode"] = render_custom_emoji_html(kwargs["caption"], pm)
        return await orig_edit_message_caption(*args, **kwargs)
...
_patch_bot_custom_emoji_methods()
...
async def send_panel_photo(
    ...
    *,
    ...
    caption, parse_mode = render_custom_emoji_html(caption, parse_mode)
...
async def send_panel_text(
    ...
    *,
    ...
    text, parse_mode = render_custom_emoji_html(text, parse_mode)
    uid = event.from_user.id
    if isinstance(event, Message):
        await delete_tracked_panel(event.chat.id, uid)
        try:
...
def init_db():
    ...
    cur.execute('''CREATE TABLE IF NOT EXISTS button_registry
...
def _find_legacy_db_candidates() -> list[str]:
    ...
    # Подхватываем типичные старые копии после обновлений/переездов.
    for folder in {os.getcwd(), BASE_DIR, os.path.join(BASE_DIR, ".."), os.path.join(BASE_DIR, "..", "..")}:
        try:
            for name in os.listdir(folder):
                low = name.lower()
                if low.startswith("bot_data") and low.endswith(".db"):
...
def button_key_from_label(label: str) -> str:
    base = (label or "").lower().strip()
    digest = hashlib.sha1(base.encode("utf-8")).hexdigest()[:16]
    return f"btn_{digest}"


def register_button_label(label: str) -> None:
    clean = (label or "").strip()
    key = button_key_from_label(clean)
    cur.execute(
        "INSERT OR REPLACE INTO button_registry (btn_key, label, last_seen) VALUES (?, ?, ?)",
        (key, clean, int(time.time())),
    )
    db.commit()


def get_button_emoji_id_for_label(label: str) -> str | None:
    key = button_key_from_label(label)
    value = (get_main_setting(f"btnemoji_{key}", "") or "").strip()
...
async def _show_admin_panel(event, edit=False):
    ...
    kb.button(text="✨ Прем-эмодзи кнопок", callback_data="adm_btnemoji_menu")
...
def _load_registered_buttons() -> list[tuple[str, str]]:
    rows = db_fetchall(
        "SELECT btn_key, label FROM button_registry ORDER BY last_seen DESC, label ASC",
        (),
    )
    out = []
    for btn_key, label in rows:
        clean = (label or "").strip()
        if clean and clean not in {"⬅️ Назад", "Назад"}:
            out.append((btn_key, clean))
    return out


def _btnemoji_page_kb(page: int, per_page: int = 20):
    all_buttons = _load_registered_buttons()
    total = len(all_buttons)
    pages = max(1, (total + per_page - 1) // per_page)
    page = max(0, min(page, pages - 1))

    start = page * per_page
    part = all_buttons[start:start + per_page]

    kb = InlineKeyboardBuilder()
    for btn_key, label in part:
        cur_id = (get_main_setting(f"btnemoji_{btn_key}", "") or "").strip()
        suffix = f" [{cur_id}]" if cur_id else " [без premium]"
        kb.button(text=f"{label}{suffix}", callback_data=f"adm_btnemoji_pick_{btn_key}")

    if page > 0:
        kb.button(text="⬅️ Стр. назад", callback_data=f"adm_btnemoji_page_{page - 1}")
    if page < pages - 1:
        kb.button(text="Стр. вперёд ➡️", callback_data=f"adm_btnemoji_page_{page + 1}")
    kb.button(text="⬅️ Назад", callback_data="adm_panel")
    kb.adjust(1)
    return kb, page, pages, total


@dp.callback_query(F.data == "adm_btnemoji_menu")
async def adm_btnemoji_menu(call: types.CallbackQuery, state: FSMContext):
    if call.from_user.id != ADMIN_ID: return
    await state.clear()
    kb, page, pages, total = _btnemoji_page_kb(0)
    await call.message.edit_text(
        "✨ **Прем-эмодзи кнопок**\n\n"
        f"Найдено кнопок: **{total}**\n"
        f"Страница: **{page + 1}/{pages}**\n\n"
        "Выберите кнопку для изменения. Отправьте `custom_emoji_id` или `0` для удаления.\n"
        "Обычные эмодзи в кнопках автоматически убраны.",
        reply_markup=kb.as_markup(),
        parse_mode="Markdown",
    )


@dp.callback_query(F.data.startswith("adm_btnemoji_page_"))
async def adm_btnemoji_page(call: types.CallbackQuery, state: FSMContext):
    if call.from_user.id != ADMIN_ID: return
    try:
        ...
        page = 0
    await state.clear()
    kb, page, pages, total = _btnemoji_page_kb(page)
    await call.message.edit_text(
        "✨ **Прем-эмодзи кнопок**\n\n"
        f"Найдено кнопок: **{total}**\n"
        f"Страница: **{page + 1}/{pages}**",
...
@dp.callback_query(F.data.startswith("adm_btnemoji_pick_"))
async def adm_btnemoji_pick(call: types.CallbackQuery, state: FSMContext):
    if call.from_user.id != ADMIN_ID: return
    key = call.data[len("adm_btnemoji_pick_"):]
    row = db_fetchone("SELECT label FROM button_registry WHERE btn_key = ?", (key,))
    if not row:
        return await call.answer("❌ Кнопка не найдена.", show_alert=True)
    await state.update_data(btnemoji_key=key)
    label = row[0]
    current = (get_main_setting(f"btnemoji_{key}", "") or "").strip() or "не задан"
    await call.message.edit_text(
        f"🎛 Кнопка: **{label}**\n"
        f"Текущий ID: `{current}`\n\n"
        "Отправьте новый `custom_emoji_id` (только цифры) или `0` для удаления.",
        reply_markup=back_kb("adm_btnemoji_menu").as_markup(),
        parse_mode="Markdown",
    )
    await state.set_state(AdminPanelStates.adm_btnemoji_value)


@dp.message(AdminPanelStates.adm_btnemoji_value)
async def adm_btnemoji_value_set(m: Message, state: FSMContext):
    raw = (m.text or "").strip()
    d = await state.get_data()
    key = d.get("btnemoji_key")
    row = db_fetchone("SELECT label FROM button_registry WHERE btn_key = ?", (key,))
    if not row:
        await state.clear()
        return await m.answer("❌ Сессия изменения кнопки устарела. Откройте меню заново.")
    label = row[0]

    if raw == "0":
        set_main_setting(f"btnemoji_{key}", "")
        await m.answer(f"✅ Premium-эмодзи удалён для кнопки: {label}")
        await state.clear()
        return

    if not raw.isdigit():
        return await m.answer("⚠️ Введите корректный `custom_emoji_id` (только цифры) или `0`.")

    set_main_setting(f"btnemoji_{key}", raw)
    await m.answer(
        f"✅ Для кнопки **{label}** установлен premium-эмодзи ID:\n`{raw}`",
        parse_mode="Markdown",
    )
    await state.clear()
...
@dp.message(F.text.in_({"📱 ТгАккаунт", "ТгАккаунт"}))
...
@dp.message(F.text.in_({"📘 Правила", "Правила"}))
...
async def profile_menu(event: Message | types.CallbackQuery, state: FSMContext):
    ...
    kb.button(text="💳 СБП (вручную)", url="https://t.me/stv18")
    ...
    await send_panel_photo(
        ...
        caption=f"👤 **Профиль**\n\n💳 Ваш баланс: **${bal}**",
        reply_markup=kb.adjust(1, 1, 2, 1).as_markup(),
    )
...
# --- КАТАЛОГ ---
@dp.message(F.text.in_({"📂 Каталог аккаунтов", "Каталог аккаунтов"}))
...
# --- УПРАВЛЕНИЕ АРЕНДОЙ ---
@dp.message(F.text.in_({"🔑 Моя аренда", "Моя аренда"}))
...
async def precheck_broadcast_chats(phone: str, owner_id: int, chats_raw: str) -> list[tuple[str, str]]:
    chats = [c.strip() for c in (chats_raw or "").split(",") if c.strip()]
    if not chats:
    ...
    try:
        ...
        for chat in chats:
            try:
                chat_ref, _ = extract_chat_and_topic(chat)
                entity = await resolve_chat_entity(client, chat_ref)
                sent = await client.send_message(entity, "1")
                try:
                    await client.delete_messages(entity, [sent.id])
                except Exception as del_err:
                    failures.append((chat, f"сообщение отправилось, но не удалилось: {type(del_err).__name__}"))
            except Exception as e:
                failures.append((chat, f"{type(e).__name__}: {e}"))
    except Exception as e:
        failures.append(("—", f"не удалось открыть сессию: {type(e).__name__}: {e}"))
    finally:
        try:
            await client.disconnect()
        except Exception:
    ...
    if failures:
        owner_row = db_fetchone('SELECT owner_id FROM accounts WHERE phone = ?', (phone,))
        current_owner = owner_row[0] if owner_row else owner_id
        lines = "\n".join([f"• `{c}` — {reason}" for c, reason in failures[:30]])
        try:
            await notify_admins(
                f"⚠️ **Предпроверка чатов не пройдена**\n"
                f"📱 Номер: `{phone}`\n"
                f"👤 Пользователь: `{owner_id}`\n"
                f"🆔 Текущий арендатор: `{current_owner or '—'}`\n\n"
                f"Проблемные чаты:\n{lines}"
            )
        except Exception:
...
async def toggle_r(call: types.CallbackQuery, state: FSMContext):
    ...
    if on:
        ...
        if r:
            failures = await precheck_broadcast_chats(p, call.from_user.id, r[2] or "")
            if failures:
                return await call.answer(
                    "❌ Не удалось запустить рассылку: есть проблемные чаты. Детали отправлены администратору.",
                    show_alert=True
                )
